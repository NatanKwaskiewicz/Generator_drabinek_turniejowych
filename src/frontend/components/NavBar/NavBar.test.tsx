...
describe('NavBar', () => {
    ...
    it('logo link points to /', () => {
        renderNavBar()
        expect(screen.getByText('Tourney').closest('a')).toHaveAttribute('href', '/')
    })

    it('Browse tournaments points to /tournaments', () => {
        renderNavBar()
        expect(screen.getByText('Browse tournaments').closest('a')).toHaveAttribute('href', '/tournaments')
    })

    it('Create bracket points to /bracketGenerator', () => {
        renderNavBar()
        expect(screen.getByText('Create bracket').closest('a')).toHaveAttribute('href', '/bracketGenerator')
    })

    it('Register team points to /createTeam', () => {
        renderNavBar()
        expect(screen.getByText('Register team').closest('a')).toHaveAttribute('href', '/createTeam')
    })
